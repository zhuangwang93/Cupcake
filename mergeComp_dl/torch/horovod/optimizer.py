...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, named_parameters, compression, compress_config=(1, 0),
        ...
        self.iterations = 0
        self.compression_start_iteration = max(compress_config[0], 1)
    ...
    def _compression_iteration(self):
        if self.iterations < self.compression_start_iteration:
            return False
        else:
            if self.uncompression_frequency > 0 and self.iterations % self.uncompression_frequency == 0:
                return False
            else:
                return True
    ...
    def _communicate_grad_async(self, p):
        ...
        if self._compression_iteration():
            ...
            if self.iterations == 0:
                # record the tensors in the backpropagation order
                self._compression.memory.add_tensor(name, tensor.numel())
    ...
    def synchronize(self):
        ...
        for p, (handles, ctx) in self._handles.items():
            if isinstance(p, tuple):
                ...
                if handles:
                    if self._compression_iteration():
                        self._compression.receive_step(handles, ctx)
                    else:
    ...
    def _update_adam_variance(self):
        """
        In some compression algorithms for Adam, such as 1-bit Adam,
        exp_avg_sq is only updated in the first iterations and then keeps constant.
        In our design, exp_avg_sq is updated in Adam.step() and
        we need to take the values out for tensor pools
        """

        # assert self.compression_start_iteration > 0
        # TODO: if need to set variances for 1bit-Adam multiple times
        if self.iterations == self.compression_start_iteration - 1:
            for group in self.param_groups:
                for p in group['params']:
                    if p.grad is not None:
                        name = self._parameter_names.get(p)
                        state = self.state[p]
                        self._compression.memory.set_variances(name, state['exp_avg_sq'])
    ...
    def step(self, closure=None):
        if self._should_synchronize:
            ...
            self.synchronize()
        self._synchronized = False
        # layer-wise and model-wise require step()
        if self._compression.memory.fusion_num <= 1:
            super(self.__class__, self).step(closure)

        if self.name == "Adam":
            self._update_adam_variance()
        self.iterations += 1
...
def DistributedOptimizer(optimizer, named_parameters=None,
    ...
    if op != Adasum or size() == 1:
        ...
        return cls(optimizer.param_groups, named_parameters, compression, compress_config, backward_passes_per_step, op,
